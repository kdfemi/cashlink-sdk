...
const iFrameBuilder = (src: string, showFrame = true) => {
    const modalZIndex = '99999999999';
    const iframe = document.createElement('iframe');
    const loader = document.createElement('div');
    loader.innerHTML = `
        <div id="${config.loaderId}" style="position:absolute; display: flex; justify-content: center; align-items: center; width: 100%; top: 0; left:0; height: 100%; background-color: rgba(0,0,0,0.5)">
    ...
    zIndexRange().then(data => {
        ...
        if(data.length) {
            ...
            if(highestZIndex > +modalZIndex) {
                modal.style.zIndex = (highestZIndex + 1).toString();
            }
        }
    }).catch();
    if(showFrame) {
